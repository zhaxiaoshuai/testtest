    ...
    <div class="apply-container">
      ...
      <div class="service-type">
        ...
        <div class="type">
          ...
          <a class="return" v-if="!isCancel" :class="[type === 'goods' && 'active']" @click="type = 'goods'">退货</a>
        </div>
      ...
      <div class="item-sales refund-money">
        ...
        <span class="price" v-if="isCancel">￥{{ returnForm.order_price | unitPrice }}</span>
        <span class="price" v-else>￥{{ returnForm.return_money | unitPrice }}</span>
      </div>
      <!--退款金额 end-->
      <!--积分抵扣 start-->
    ...
    mounted() {
      ...
      API_AfterSale.getAfterSaleData(this.order_sn, this.sku_id).then(response => {
        ...
        this.returnForm.return_money = Foundation.formatPrice(refund_price * num)
	      // 取消订单时 退款包含运费 取order_price字段的值
        this.returnForm.order_price = Foundation.formatPrice(response.order.order_price)
